  ...
  return (
    <section className="relative py-20 md:py-28 overflow-hidden bg-[#0A0A0A]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-6 md:gap-8">
            {matchdayStats.map((stat, i) => (
              <div key={stat.label} className="flex items-center gap-3">
                {i > 0 && (
                  <div className="w-px h-5 bg-white/10 -ml-3 md:-ml-4 mr-0" />
                )}
                <div>
                  <span className="font-oswald text-2xl md:text-3xl font-bold text-white tracking-tight leading-none">
                    {stat.value}
                  </span>
                  <span className="text-[11px] font-medium uppercase tracking-wider text-white/40 ml-2">
                    {stat.label}
                  </span>
                </div>
              </div>
        ...
        <FadeIn threshold={0.1} className="mb-8">
          <div className="relative aspect-video rounded-xl overflow-hidden bg-black border border-white/10 shadow-2xl shadow-black/40">
            {playing ? (
              <iframe
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-red-600 flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        {/* Player outcomes — featured + supporting */}
        <FadeIn delay={100}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          ...
          <FadeIn delay={150} className="lg:col-span-2">
            <div className="h-full rounded-xl border border-white/10 bg-white/[0.05] p-6 md:p-8 flex flex-col">
              {featured.outcome && (
                <span className="inline-block w-fit text-[11px] font-bold uppercase tracking-widest text-red-400 border border-red-600/25 rounded-full px-3 py-1 mb-5">
                  {featured.outcome}
                </span>
              )}
              ...
              <div className="pt-4 border-t border-white/[0.06] flex items-end justify-between gap-4">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-white/20 shrink-0">
                  Verified
                </span>
              </div>
            </div>
          ...
          {/* Supporting results — stacked in 1 col */}
          <div className="flex flex-col gap-6">
            {supporting.map((story, i) => (
              <FadeIn key={story.name} delay={200 + i * 80}>
                <div className="rounded-xl border border-white/10 bg-white/[0.05] p-6 h-full flex flex-col">
                  {story.outcome && (
                    <span className="inline-block w-fit text-[11px] font-bold uppercase tracking-widest text-red-400/80 border border-red-600/20 rounded-full px-3 py-1 mb-4">
                      {story.outcome}
                    </span>
                  )}
        ...
        <FadeIn delay={250}>
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] px-6 py-5 md:px-8 md:py-6">
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
              <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/30 shrink-0">
                After Every Match
              </span>
