-- Fix: Allow re-enrollment by updating existing stopped/completed enrollments
-- instead of trying to insert new ones (which violates unique constraint)
...
BEGIN
  ...
  LOOP
    ...
    IF existing_enrollment IS NOT NULL THEN
      -- If already active or paused, skip (don't re-enroll)
      IF existing_enrollment.status IN ('active', 'paused') THEN
        CONTINUE;
      END IF;

      -- If stopped or completed, reset and re-enroll
