...
/**
 * Server-side proxy for brochure view tracking. Forwards to the CRM's
 * /api/public/brochure/view with the shared secret so a page open bumps the
 * view counter. Best-effort — fired by the viewer when the flipbook is ready.
 *
...
export async function POST(request: NextRequest) {
  ...
  if (!crmUrl || !secret) {
    console.error("Brochure view proxy not configured: missing IFG_CRM_URL or IFG_FORM_INGEST_SECRET");
  ...
  try {
    const res = await fetch(`${crmUrl.replace(/\/$/, "")}/api/public/brochure/view`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: (data as { error?: string }).error || "Could not record the view." },
        { status: res.status },
      );
    }
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("Brochure view proxy error:", err);
